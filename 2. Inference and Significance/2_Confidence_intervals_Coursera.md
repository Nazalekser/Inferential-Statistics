...
```r
set.seed(9166015)                 # make sure to change the seed
```
...
![plot of chunk plot-ci](figure/plot-ci-1.png)
...
cis <- cis %>%
  mutate(capture_mu = ifelse(lower < params$mu & upper > params$mu, "yes", "no"))
cis_data <- data.frame(ci_id = c(1:50, 1:50),
                      ci_bounds = c(cis$lower, cis$upper),
                      capture_mu = c(cis$capture_mu, cis$capture_mu))
ggplot(data = cis_data, aes(x = ci_bounds, y = ci_id, 
                           group = ci_id, color = capture_mu)) +
  geom_point(size = 2) +  # add points at the ends, size = 2
  geom_line() +           # connect with lines
  geom_vline(xintercept = params$mu, color = "darkgray") # draw vertical line
...
![plot of chunk plot-99-perc-cis](figure/plot-99-perc-cis-1.png)
